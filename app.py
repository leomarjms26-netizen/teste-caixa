...
# 🔐 Lê o segredo direto do ambiente do GitHub Actions
INTEGRACAOGOOGLESHEET = os.environ["INTEGRACAOGOOGLESHEET"]

# Caso o segredo venha codificado em Base64, decodifica automaticamente
try:
    cred_data = json.loads(INTEGRACAOGOOGLESHEET)
except json.JSONDecodeError:
    import base64
    decoded = base64.b64decode(INTEGRACAOGOOGLESHEET).decode("utf-8")
...
def autenticar_google():
    try:
        # 🔑 Cria as credenciais diretamente a partir do segredo JSON
...
def atualizar_porta(creds, linha, porta):
    try:
        ...
        body = {"values": [["SIM", "", f"SIM, {data_atual}"]]}
...
if 'portas' in st.session_state:
    ...
    if not portas:
        ...
        for linha, row in portas:
            for label, valor in [("CABO", row[0]), ("PRIMARIA", row[1]),
                                 ("CAIXA", row[2]), ("PORTA", row[4]),
                                 ("CAPACIDADE", row[5]), ("INTERFACE", row[6])]:
                st.markdown(f"""
                <div class="div-campo">
